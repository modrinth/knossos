...
(async () => {
    ...
    // Write JSON file
    await fs.writeFile(
        GENERATED_PATH + "gameVersions.json",
    ...
    const modCount = (
        await (
            await fetch(
                API_URL + 'search?limit=0&facets=[["project_type:mod"]]'
            )
        ).json()
    ).total_hits;
        ...
        mods = [
            ...
            ...((
                await (
                    await fetch(
                        API_URL +
                            `search?limit=100&facets=[["project_type:mod"]]&offset=${
                                i * 100
                            }`
                    )
                ).json()
    ...
    const otherCount = (
        await (
            await fetch(
                API_URL + "search?limit=0&filters=project_type!%3D%22mod%22"
            )
        ).json()
    ).total_hits;
        ...
        others = [
            ...
            ...(
                await (
                    await fetch(
                        API_URL +
                            `search?limit=100&filters=project_type!%3D%22mod%22&offset=${
                                i * 100
                            }`
                    )
                ).json()
            ...
            .map(async (project, index) => {
                try {
                    ...
                    const iconBuffer = Buffer.from(
                        await (await fetch(project.icon_url)).arrayBuffer()
                    );
    ...
    // Write JSON file
    await fs.writeFile(
        GENERATED_PATH + "homepage.json",
        JSON.stringify({ random: Math.random() })
    );
