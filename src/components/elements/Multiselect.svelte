<script lang="ts">
    ...
    export let selected: Option = options.find(
    ...
    const minWidth =
        options
            ...
            .reduce((it, acc) => (it.toString().length > acc ? it : acc), "")
            .toString().length * 9;
...
<div
    class="multiselect multiselect--color-{color}"
    class:is-open={open}
    class:multiselect--opens-up={shouldOpenUp}
    use:clickOutside={() => {
        open = false;
    }}
    bind:this={element}
...
>
    ...
    >
        ...
        {/if}
        <span class="multiselect__input__value" style:min-width="{minWidth}px"
