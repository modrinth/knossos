<script lang="ts">
    ...
    export let icon: string | SvelteComponent | typeof SvelteComponentDev = "";
    export let type: "text" | "select" | "checkboxes" | "textarea" | "checkbox";
    export let options: {
        label: string;
        value: string | number;
        icon?: string | typeof SvelteComponentDev;
    }[] = [];
...
{/if}
{#if type === "text"}
    <input type="text" name={key} class="input-box" {placeholder} bind:value />
{:else if type === "select"}
...
{:else if type === "textarea"}
    <textarea
        name={key}
        class="input-box input-box--fill"
        {placeholder}
        bind:value
    />
