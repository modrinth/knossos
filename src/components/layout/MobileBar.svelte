...
<div class="mobile-bar">
    ...
    <div class="mobile-bar__nav" use:clickOutside={() => (showMenu = false)}>
        <a
            ...
            href="/"
            style:--color-brand={$page.url.pathname === "/"
                ? ""
                : "var(--color-text-lightest)"}
        >
            <LogoIcon />
        </a>
    ...
    <div class="mobile-bar__menu" class:is-shown={showMenu}>
            ...
            {#each dropdownItems.filter((it) => !it.divider && (!it.id || it.id !== "create")) as item}
                {#if item.href}
                    <a class="mobile-bar__menu__item" href={item.href}>
                        ...
                        {#if item.sublabel}
                            <span class="mobile-bar__menu__item__sublabel"
                                >{item.sublabel}</span
                            >
                        {/if}
                    </a>
                {:else}
                    <button
                        class="mobile-bar__menu__item {item?.id === 'logout'
                            ? 'mobile-bar__menu__item--color-red'
                            : ''}"
                    >
                        <span class="mobile-bar__menu__item__title">
                            <svelte:component this={item.icon} />
        ...
        {:else}
            <a
                class="mobile-bar__menu__item mobile-bar__menu__item--color-brand"
                href={browser
