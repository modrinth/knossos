...
<div class="single-layout">
  <div class="title-bar">
    ...
    {#if !$loaded || $notifications.length > 0}
      <Button label="Clear all" color="brand" icon={IconTrash} on:click={async () => {
        await send('DELETE', `notifications?ids=${JSON.stringify($notifications.map(it => it.id))}`)
        $notifications = []
      }}/>
    {/if}
  </div>
      ...
      <div class="card notification">
        ...
        <div class="notification__actions">
          {#if notification.type === 'project_update'}
            <Button label="Clear" icon={IconTrash} on:click={async () => {
          await deleteNotification(notification.id)
    }}/>
          {/if}
          {#each notification.actions as action}
            <Button label={action.title} on:click={async () => {
            try {
