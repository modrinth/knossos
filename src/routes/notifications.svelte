        ...
        {#if !$loaded || $notifications.length > 0}
            <Button
                ...
                on:click={async () => {
                    await send(
                        "DELETE",
                        `notifications?ids=${JSON.stringify(
                            $notifications.map((it) => it.id)
                        )}`
                    );
                    $notifications = [];
                }}
                ...
                <div class="notification__info">
                    <div class="notification__info__title">
                        ...
                        <span class="notification__info__title__time">
                            {@html $t("stats.notified", {
                                values: { ago: ago(notification.created) },
                            })}
                        </span>
                    </div>
                    ...
                    {#each notification.actions as action}
                        <Button
                            ...
                            on:click={async () => {
                                try {
                                    await send(
                                        action.action_route[0],
                                        action.action_route[1]
                                    );
                                } catch {
                                    // Workaround for https://github.com/modrinth/labrinth/issues/279
                                    console.warn(
                                        "Could not perform notification action."
                                    );
                                }
                                await deleteNotification(notification.id);
                            }}
