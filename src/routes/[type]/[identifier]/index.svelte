...
<Meta
    title="{$project.title} - Minecraft {$t(
        `project.types.${$project.project_type}.singular`
    )}"
...
<div class="card project-body card--gap-none" style:--padding="0">
    <div class="card__overlay card__overlay--row">
        {#if $permissions.data.editBody}
            {#if isEditing}
                {#if dirty}
                    <Button
                        label={$t("generic.actions.discard")}
                        icon={IconTrash}
                        on:click={() => {
    ...
    {:else}
        <div class="project-body__text text">
            ...
            {:else}
                <p class="project-body__text__placeholder">
                    This project description is empty.
                </p>
            {/if}
        </div>
    {/if}
