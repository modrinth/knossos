<script context="module" lang="ts">
    export async function load({ params, fetch, session, stuff }) {
        const version = stuff.versions.find((it) =>
            [it.version_number, it.id].includes(params.version)
        );
        ...
        if (version) {
            ...
            return {
                status: 404,
                error: new Error(
                    `The version you were looking for cannot be found.`
                ),
            };
        }
    }
    ...
    <div class="button-group">
        ...
        {:else}
            <Button
                ...
                color="raised"
                icon={IconFlag}
                on:click={() => report("version", version.id)}
            />
        {/if}
        ...
        <div class="card">
            ...
            {#each version.files as file}
                <a
                    ...
                    class:file--primary={file.primary ||
        ...
        {#if versionDependencies.length > 0}
            <div class="card">
                ...
                {#each versionDependencies as dep}
                    <a
                        class="dependency"
                        href="/{dep.project.project_type}/{dep.project.slug ||
                            dep.project.id}{dep.version
                    ...
                    >
                        ...
                        <div class="dependency__info">
                            ...
                            {#if dep.version}
                                <p>
                                    Version {dep.version.version_number} is {dep.dependency_type}
                                </p>
                            {/if}
                        </div>
                    </a>
        ...
        <div class="card">
            ...
            <div>
                ...
                <a class="member" href="/user/{publisher.user.username}">
                    <ProfilePicture
                        src={publisher.user.avatar_url}
                        size="sm"
                        circle
                    />
                    <div class="member__info">
                        <span class="member__info__link"
                            >{publisher.user.username}</span
                        >
