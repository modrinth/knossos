    ...
    export async function load({ params, fetch, session, stuff }) {
        try {
            let [projects, reports] = await Promise.all([
                (
                    await send("GET", `moderation/projects`, null, { fetch })
                ).map((it) => ({ ...it, moderation_type: it.project_type })),
                (
                    await send("GET", `report`, null, { fetch })
                ).map((it) => ({ ...it, moderation_type: "report" })),
            ]);
                ...
                reports.map(async (report) => {
                    try {
                        ...
                        if (report.item_type === "user") {
                            ...
                            report.item_id =
                                version.version_number || report.item_id;
