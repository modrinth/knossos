...
onMounted(async () => {
  ...
  const body = `project_ids=${projectIds}&start_date=${new Date(new Date() - 30 * 24 * 60 * 60 * 1000).toISOString()}`
  const longTerm = `project_ids=${projectIds}&start_date=${new Date(new Date().getFullYear(), 0, 1).toISOString()}&resolution_minutes=43200`
...
const processRegionData = (analytics) => {
  ...
  const finalData = {
    labels: Array.from(projectSet).map(project => props.projects.find(proj => proj.id === project).title),
    data: [],
