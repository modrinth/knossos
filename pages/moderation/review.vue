  ...
  <div>
    ...
    <section class="universal-card">
      ...
      <div class="input-group">
        <Chips v-model="projectType" :items="projectTypes" :format-label="(x) => x === 'all' ? 'All' : $formatProjectType(x) + 's'" />
        <button v-if="oldestFirst" class="iconified-button push-right" @click="oldestFirst = false"><SortDescIcon />Sorting by oldest</button>
        <button v-else class="iconified-button push-right" @click="oldestFirst = true"><SortAscIcon />Sorting by newest</button>
      </div>
      <div
        v-for="project in projects.sort((a, b) => {
          ...
          }).filter((x) => projectType === 'all' || app.$getProjectTypeForUrl(x.project_type, x.loaders) === projectType)"
        :key="`project-${project.id}`"
        class="universal-card recessed project"
      >
        <div class="title-row">
          <nuxt-link :to="`/${project.inferred_project_type}/${project.slug}`" class="iconified-stacked-link project-title">
        ...
        <div class="input-group">
          <nuxt-link :to="`/${project.inferred_project_type}/${project.slug}`" class="iconified-button raised-button"><EyeIcon /> View project</nuxt-link>
          <button class="iconified-button brand-button" @click="setProjectStatus(project, project.requested_status ? project.requested_status : 'approved')"><CheckIcon /> Approve</button>
          <button class="iconified-button danger-button" @click="setProjectStatus(project, 'withheld')"><EyeOffIcon /> Withhold</button>
          <button class="iconified-button danger-button" @click="setProjectStatus(project, 'rejected')"><CrossIcon /> Reject</button>
        </div>
        <span v-if="project.queued" :class="`submitter-info ${project.age_warning}`">
          <WarningIcon v-if="project.age_warning" />
          Submitted for review
          <span v-tooltip="$dayjs(project.queued).format('MMMM D, YYYY [at] h:mm A')">{{ fromNow(project.queued) }}</span>
        </span>
...
rawProjects.map((project) => {
  project.owner = rawMembers.flat().find((x) => x.team_id === project.team && x.role === 'Owner').user
  project.age = project.queued ? (now - app.$dayjs(project.queued)) * 3 : Number.MAX_VALUE
  project.age_warning = ''
